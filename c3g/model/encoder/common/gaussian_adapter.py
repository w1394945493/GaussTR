...
class UnifiedGaussianAdapter(GaussianAdapter):
    def forward(
        ...
        raw_gaussians: Float[Tensor, "*#batch _"],
        eps: float = 1e-8,
        intrinsics: Optional[Float[Tensor, "*#batch 3 3"]] = None, # todo 相机内外参 None
        coordinates: Optional[Float[Tensor, "*#batch 2"]] = None,
        features = None,
    ) -> Gaussians:
        scales, rotations, sh = raw_gaussians.split((3, 4, 3 * self.d_sh), dim=-1)

        scales =self.scale_weight * F.softplus(scales)
        ### WHAT IS THIS
        if self.clamping > 0:
            scales = scales.clamp_max(self.clamping)

        if self.isotrophic_covariance:
        ...
        opacities = torch.max(opacities, torch.tensor(self.opacity_min))
