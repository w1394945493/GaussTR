...
class VolumeDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 render_type = '3dgs',
                 image_shape = (384,640),
                 render_shape = (180,320),
                 semantic_sample_ratio = 1.0,
                 use_fp16 = True,
        ...
        self.semantic_sample_ratio = semantic_sample_ratio
        ...
        class_weights = torch.from_numpy(1 / np.log(nusc_class_frequencies[:num_classes] + 0.001)) # todo 为各类目标设定了不同的权重
        self.cls_weights = class_weights

        self.register_buffer('xyz_min', torch.from_numpy(
            np.array([self.real_size[0], self.real_size[2], self.real_size[4]])))
        self.register_buffer('xyz_max', torch.from_numpy(
            np.array([self.real_size[1], self.real_size[3], self.real_size[5]])))


        self.ZMAX = self.real_size[1]

        self.Z = self.voxels_size[0]
        self.Y = self.voxels_size[1]
        self.X = self.voxels_size[2]
        ...
        if self.contracted_coord:
            ...
            self.norm_func = lambda xyz: vox.world2contracted(xyz, pc_range_roi=pc_range_roi, ratio=self.contracted_ratio)
        ...
        self.bounds = (self.real_size[0], self.real_size[1],
        ...
        self.vox_util = vox.Vox_util(
            ...
            real_size = real_size,
            contracted_coord = contracted_coord,
            contracted_ratio = contracted_ratio,
            assert_cube=False)


        activate_fun = nn.ReLU(inplace=True)
        out_channel = 1
        self._3DCNN = S3DCNN(input_planes=input_channel, out_planes=out_channel, planes=con_channel,
    ...
    def get_density(self, Voxel_feat, is_train, inputs, cam_num):
        ...
        if 'gs' in self.render_type:
            semantic = None
            reg_loss = {}
            # todo 获取高斯属性：opacity scaleing rotation
            # todo 论文3.2 每个高斯:使用固定尺度[1 1 1]和固定旋转[1 0 0 0],通过优化其余的高斯属性(语义和不透明度)来建模三维场景
            K, C2W, pc = self.prepare_gs_attribute(Voxel_feat, inputs) # todo pc: 'get_rotation': (N 4): [1 0 0 0] 'get_opacity': (N 1) 'get_features':(N 3): [1 1 1]

            # pdb.set_trace()
            depth, rgb_marched = self.get_splatting_rendering(K, C2W, pc, inputs)

            if self.use_semantic: # todo 语义图
    ...
    def prepare_gs_attribute(self, Voxel_feat_list, inputs, is_train = None, index = 0):
        ...
        if self.gs_sample != 0:
            ...
            vox_grid = vox_grid.squeeze(0)
            pc['get_xyz_grid'] = vox_grid
            out_channel = Voxel_feat_list.shape[1] # todo 18
        ...
        # pdb.set_trace()
        if self.use_semantic:
            pc['semantic'] = gs_attribute # (300x300x24,18)

        pc['get_features'] = torch.ones_like(pc['get_opacity']).repeat(1, 3) # (N,3) # todo [1 1 1]
    ...
    def get_splatting_rendering(self, K, C2W, pc, inputs, flow_index = (0, 0)):
        ...
        # depth
        for j in range (C2W.shape[0]): # todo 各个相机的
    ...
    def forward(self, features, inputs, epoch = 0, outputs={}, is_train=True, Voxel_feat_list=None, no_depth=False):
        ...
        for scale in self.scales: # todo 0
            ...
            eps_time = time.time()
            # todo -------------------------------#
            # todo 在体素空间上进行 gaussian splat, 与传统的体积渲染(NeRF)方法相比，速度更快
        ...
        if not is_train:
            ...
            ret = self.grid_sampler(xyz, Voxel_feat_list[0], vis=True) # ret[0]: (b,200x200x16) ret[1]: None ret2: (b,200x200x16,18) 语义occ

            if self.use_semantic:
                pred_occ_logits = ret[2] # todo 语义
            else:
