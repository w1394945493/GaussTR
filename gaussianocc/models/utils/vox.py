...
class Vox_util(nn.Module):
    def __init__(self, Z, Y, X, scene_centroid, bounds, position = 'embedding', length_pose_encoding = 3,
                 real_size = [-40.0, 40.0, -40.0, 40.0, -1.0, 5.4],
                 contracted_coord = True,
                 contracted_ratio = 0.66667,
                 gs_sample = 0.0,
                 B = 1,
                 pad=None, assert_cube=False):
        ...
        self.pc_range_roi = [self.real_size[0], self.real_size[2], self.real_size[4], \
                             self.real_size[1], self.real_size[3], self.real_size[5]]  # [x_min, y_min, z_min, x_max, y_max, z_max]
    ...
    def Mem2Ref(self, xyz_mem, Z, Y, X, assert_cube=False):
        ...
        if self.contracted_coord:
            ...
            xyz_ref = contracted2world(xyz_norm, pc_range_roi=self.pc_range_roi, ratio=self.contracted_ratio)
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False):
        ...
        xyz_memA = basic.gridcloud3d(B, Z, Y, X, norm=False, device=pixB_T_camA.device) # todo 生成3D体素网格点: (b,300x300x14,3) 每个体素的中心坐标
        # y is height here
        # todo 2. 将体素坐标从memory坐标系 -> 世界坐标系
        xyz_camA = self.Mem2Ref(xyz_memA, Z, Y, X, assert_cube=assert_cube)  # xyz in world coordinates # todo 将体素坐标变换到真实空间

        # todo 3. 世界坐标 -> 相机B坐标：将体素坐标变换到相机坐标系中
        xyz_camB = geom.apply_4x4(camB_T_camA, xyz_camA)  # xyz in camera coordinates
        z = xyz_camB[:,:,2]  # z is depth # todo 用于后续的判断：判断体素是否在相机前方

        # todo 4. 世界坐标 -> 像素坐标：得到每个体素到相机B的像素位置
        ...
        x, y = xy_pixB[:,:,0], xy_pixB[:,:,1]                   # todo 判断是否落在像素内
        # these are B x N
        # todo 5. 判断体素能否被相机看到
        x_valid = (x>-0.5).bool() & (x<float(W-0.5)).bool() # todo 体素在像素坐标系下坐标：表述这个点在图像宽度/高度范围内
        y_valid = (y>-0.5).bool() & (y<float(H-0.5)).bool()
        z_valid = (z>0.0).bool() # todo 体素在相机坐标系下的坐标
        valid_mem = (x_valid & y_valid & z_valid).reshape(B, 1, Z, Y, X).float() # todo 定义了哪些体素是否被相机看到
        ...
        z_pixB = torch.zeros_like(x) # todo 第三维z_pixB设置为0
        xyz_pixB = torch.stack([x_pixB, y_pixB, z_pixB], axis=2)

        # todo 7. 使用grid_sample 从 2D feature map中取值：从2D图像特征中，按照每个voxel在图像中对应的位置采样特征
        rgb_camB = rgb_camB.unsqueeze(2)
        xyz_pixB = torch.reshape(xyz_pixB, [B, Z, Y, X, 3]).to(rgb_camB.device)
        values = F.grid_sample(rgb_camB, xyz_pixB, align_corners=False) # (N, H, W, Z, 3) # todo 从2D图像中采样特征值：f: (b,C,1,h,w) 坐标：(b,z,y,x,3)

        # todo 8. 过滤无效体素 & 返回 3D特征图
        values = torch.reshape(values, (B, C, Z, Y, X))
        # 16, 256, 256
        values = values * valid_mem.to(values.device)
    ...
    def get_voxel_grid(self, cam_center, assert_cube=False):
        ...
        if self.gs_sample == 0:
            ...
            Z, Y, X = int(self.gs_sample*self.Z), int(self.gs_sample*self.Y), int(self.gs_sample*self.X)

        xyz_memA = basic.gridcloud3d(self.B, Z, Y, X, norm=False, device=cam_center.device)  # 定义voxel大小
    ...
    def get_meta_data(self, cam_center, camB_T_camA = None, abs_position=False, assert_cube=False):
        ...
        Z, Y, X = self.Z, self.Y, self.X
        xyz_memA = basic.gridcloud3d(self.B, Z, Y, X, norm=False, device=cam_center.device)  #
        xyz_camA = self.Mem2Ref(xyz_memA, Z, Y, X, assert_cube=assert_cube)  # re-center
