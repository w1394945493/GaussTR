...
class OccMetric(BaseMetric):
    ...
    def process(self, data_batch, data_samples):
        ...
        if self.occ_flag:
            # todo -------------------------------#
            # todo 占用预测评估
            preds = data_samples[0]['occ_pred'] # todo (b 200 200 16)
            labels = torch.cat(
                [d.gt_pts_seg.semantic_seg for d in data_batch['data_samples']]) # todo (b 200 200 16)
            if self.use_image_mask: # todo True 有用到image掩码
                mask = torch.stack([
                    torch.from_numpy(d.mask_camera)
                    for d in data_batch['data_samples']
                ]).to(labels.device, torch.bool)
            elif self.use_lidar_mask: # todo False 未用到
                mask = torch.stack([
