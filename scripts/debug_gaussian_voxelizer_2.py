...
def _splat_bwd_kernel_opt(
    grad_features_ptr, # 输出：高斯特征的梯度
    grid_density_ptr,  # 前向推理：网格密度
    grad_grid_feats_ptr,    # 损失对网格特征的梯度
    means_ptr, inv_covs_ptr, opacities_ptr, radii_ptr,
    vol_min_x, vol_min_y, vol_min_z,
    voxel_size,
    N_gaussians,n_dims, 
    dim_x, dim_y, dim_z,
    ...
    for x in range(ix_start, ix_end): 
        ...
        for y in range(iy_start, iy_end):
            ...
            for z in range(iz_start, iz_end):
                ...
                # 梯度计算：将网格梯度累加到高斯特征梯度
                for f in range(n_dims):
                    grad_grid_feat = tl.load(grad_grid_feats_ptr + offset * n_dims + f)
                    grad_gauss_feat = grad_grid_feat * density / grid_density_val
...
class GaussSplatting3D(torch.autograd.Function):
    ...
    def forward(ctx, means3d, covs, opacities, features, vol_range, voxel_size, grid_shape):
        ...
        # 存储必要的张量
        ctx.save_for_backward(means3d, opacities, features, inv_covs, radii, grid_density)
        # 存储普通数值
    ...
    def backward(ctx, grad_grid_density, grad_grid_feats): # 损失对前向输出的梯度
        # 恢复前向向量
        means3d, opacities, features, inv_covs, radii, grid_density = ctx.saved_tensors
        N = means3d.shape[0]
        n_dims = features.shape[1]        
        ...
        _splat_bwd_kernel_opt[(N,)](
            ...
            means3d, inv_covs.reshape(N, 9), opacities, radii,
        ...
        return None, None, None, grad_features, None, None, None
...
if __name__=='__main__':
    ...
    torch.cuda.synchronize()
    t0 = time.time()
    grid_density_triton, grid_feats_triton = GaussSplatting3D.apply(means3d, covs, opacities, features_triton, vol_range, voxel_size, grid_shape)
    t1 = time.time()   
    torch.cuda.synchronize()
    ...
    grad_triton = features_triton.grad
    ...
    loss_ori.backward()
    grad_ori = features_ori.grad
    
    # 3. 对比梯度
    try:
        torch.testing.assert_close(grad_triton, grad_ori, rtol=1e-4, atol=1e-5)
        print("Backward 梯度验证 -> [通过]")
