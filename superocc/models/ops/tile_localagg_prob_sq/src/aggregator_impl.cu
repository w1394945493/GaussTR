...
int LocalAggregator::Aggregator::forward(
	...
	bool debug)
{
	dim3 tile_grid((W + BLOCK_X - 1) / BLOCK_X, (H + BLOCK_Y - 1) / BLOCK_Y, (D + BLOCK_Z - 1) / BLOCK_Z);
	...
	size_t img_chunk_size = required<ImageState>(tile_grid.x * tile_grid.y * tile_grid.z);
	char* img_chunkptr = imageBuffer(img_chunk_size);
	ImageState imgState = ImageState::fromChunk(img_chunkptr, tile_grid.x * tile_grid.y * tile_grid.z);
	...
	bool debug)
{
	...
	ImageState imgState = ImageState::fromChunk(img_buffer, tile_grid.x * tile_grid.y * tile_grid.z);
