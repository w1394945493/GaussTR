...
class LoadFeatMaps(ResizeCropFlipImage):
    ...
    def __call__(self, results):
        ...
        results[f'ori_{self.key}'] = np.array(ori_feats, dtype=np.float32) # todo ori_depth
        results[self.key] = np.array(feats,dtype=np.float32)  # todo depth
        return results
...
class BEVLoadMultiViewImageFromFiles(object):
    ...
    def __call__(self, results):
        ...
        # results['ori_img'] = deepcopy(img).transpose(3, 2, 0, 1) 
        results['ori_img'] = img[:, :, ::-1, :].transpose(3, 2, 0, 1).copy() # todo bgr -> rgb (h w 3 n) -> (n,3,h,w)
        return results
