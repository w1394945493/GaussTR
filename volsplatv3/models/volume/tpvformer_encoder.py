...
class TPVFormerEncoder(TransformerLayerSequence):
    ...
    # todo (wys 10.20) TPV(Tri-Plane View)编码器：将多视角图像特征整合成统一的三维空间特征表示
    def forward(self, mlvl_feats, project_feats, img_metas):
        ...
        tpv_queries_hw = self.tpv_embedding_hw.weight.to(dtype) # (h*w,128) 192 * 192 = 36864
        tpv_queries_zh = self.tpv_embedding_zh.weight.to(dtype) # (z*h,128) 16 * 192 = 3072
        ...
        for ref_3d in ref_3ds:
            reference_points_cam, tpv_mask = self.point_sampling(
                ...
                img_metas)  # num_cam, bs, hw++, #p, 2 # 将每个平面的参考3D坐标投影到各相机视图，得到在每个相机中对应的2D采样点
